...
`update_fields.pyx` contains code implementing the defining equations of hypoelastoplasticity (linear elasticity at the moment), and returns the *change* in each `Field` at the current timestep. The associated `update_fields.pxd` file contains definitions.

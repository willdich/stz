...
cpdef void go(int N_x, int N_y, int N_z, int N_t,                                       # Number of grid points in each dimension
    ...
    cdef:
        ...
        np.float64_t curr_sig_comp, curr_u_comp
            ...
            for yy in range(1, N_y + 1):
                for zz in range(1, N_z + 1):
                    ...
                    if ((yy == 1) and (zz == 1)):
                        ...
                        curr_sig_comp = comp_wave_sig((xx - 1) * dx, L_x, tt, lambd, mu, rho)
                        curr_u_comp = comp_wave_u((xx - 1) * dx, L_x, tt, lambd, mu, rho)

                        # And print the data to the output file
                        fprintf(fp, "%f %f %f %f %f %f %f %f %f %f\n",
                                                # We again use xx-1, yy-1, and zz-1 by the above logic
                                                tt, (xx-1)*dx, (yy-1)*dy, (zz-1)*dz,
                                                curr_grid_element.u, curr_grid_element.s11,
                                                curr_u_comp, curr_sig_comp,
                                                libc.math.pow(libc.math.fabs(curr_grid_element.u - curr_u_comp), 2),
                                                libc.math.pow(libc.math.fabs(curr_grid_element.s11 - curr_sig_comp), 2))
...
cdef void set_boundary_conditions(Field *grid,                                                  # Grid
                                  ...
                                  np.float64_t lambd,                                           # Material lambda
    ...
    for xx in range(1, N_x + 1):
        for yy in range(1, N_y + 1):
            for zz in range(1, N_z + 1):
                ...
                curr_field.u = comp_wave_u((xx - 1) * dx, L_x, 0, lambd, mu, rho)
                curr_field.s11 = comp_wave_sig((xx - 1) * dx, L_x, 0, lambd, mu, rho)

cdef np.float64_t comp_wave_u(np.float64_t xx, np.float64_t L_x, np.float64_t tt,
                               np.float64_t lambd, np.float64_t mu, np.float64_t rho) nogil:

    """ Returns the value of a velocity compressive wave located in plane x=xx at time tt """
    cdef np.float64_t c = libc.math.sqrt((lambd + 2 * mu) / rho)

    return -c / (lambd + 2 * mu) * comp_wave_sig(xx, L_x, tt, lambd, mu, rho) 

cdef np.float64_t comp_wave_sig(np.float64_t xx, np.float64_t L_x, np.float64_t tt,
                                 np.float64_t lambd, np.float64_t mu, np.float64_t rho) nogil:

    """ Returns the value of a compressive-stress compressive wave located in plane x=xx at time tt """

    cdef np.float64_t pi = 3.14159265359
    cdef np.float64_t c = libc.math.sqrt((lambd + 2 * mu) / rho)

    return libc.math.sin( 2 * pi  / L_x *(xx - c * tt))
